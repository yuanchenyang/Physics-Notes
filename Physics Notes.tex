...
\begin{align*}
  A_1'&= \gamma(A_1+\beta A_4)\\
  A_2'&= A_2\\
  A_3'&= A_3\\
  A_4'&= \gamma(A_4+\beta A_1)
\end{align*}
The dot product of two 4-vectors is invariant under Lorentz transformations:
\begin{align*}
  \fve{A}\cdot\fve{B} &= A_1B_1 + A_2B_2 + A_3B_3 - A_4B_4\\
  &= \fve{A'}\cdot\fve{B'}
\end{align*}
\subsubsection{Different 4-vectors}
\begin{description}
  \setlength{\itemsep}{-2mm}
\item[4-position] $(dx, dy, dz, cdt)$
  4-vectors originate from the invariant interval $ds$.
  \begin{align*}
    \fve{ds}^2&=(dx, dy, dz, cdt)^2 \\
    &=dx^2+dy^2+dz^2-c^2dt^2
  \end{align*}
\item[4-velocity] $\gamma_v(\ve{v},c)$
  To obtain other 4-vectors, we can multiply invariant quantities to the
  4-position vector, such as proper time:
  \begin{align*}
    d\tau&=\frac{dt}{\gamma}\\
    \fve{v}&=\frac{ds}{d\tau}\\
    &=\gamma_v\left(\frac{dx}{dt},\frac{dy}{dt},\frac{dz}{dt},c\right)\\
    &=\gamma_v(\ve{v},c)
  \end{align*}
\item[4-momentum] $\left(\ve{p},\frac{E}{c}\right)$
  As mass is invariant,
  \begin{align*}
    \fve{p}&=m\fve{v}\\
    &=(\gamma_vm\ve{v},\gamma_vmc)\\
    &=\left(\ve{p},\frac{E}{c}\right)
  \end{align*}
  For photons in x-direction, the 4-momentum vector is:
  \begin{align*}
    \fve{p}=\left(\frac{h}{\lambda},0,0,\frac{h}{\lambda}\right)
  \end{align*}
\item[4-wave] $\left(\ve{k},\frac{\omega}{c}\right)$
  For electromagnetic waves,
  \begin{align*}
    k&=\frac{2\pi}{\lambda}=\frac{\omega}{c}\\
    \ve{p}&=\frac{h}{\lambda}=\hbar\ve{k}\\
    E&=hf=\hbar\omega\\
    \fve{p}&=\hbar\left(\ve{k},\frac{\omega}{c}\right)\\
    \fve{k}&=\frac{\fve{p}}{\hbar}
  \end{align*}
\item[4-force] $\gamma_v\left(\ve{f},\frac{1}{c}\frac{dE}{dt}\right)$
  \begin{align*}
    \fve{F}&=\frac{d\fve{p}}{d\tau}\\
    &=\gamma_v\left(\ve{f},\frac{d}{dt}\left(\frac{E}{c}\right)\right)
  \end{align*}
\end{description}
